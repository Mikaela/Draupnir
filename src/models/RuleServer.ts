...
export default class RuleServer {
    ...
    private addEventRules(eventRules: EventRules): void {
        ...
        if (this.rulesByEvent.get(roomId)?.has(eventId)) {
            throw new TypeError(
                `There is already an entry in the RuleServer for rules created from the event ${eventId}.`,
            );
        }
    ...
    private stopEventRules(eventRules: EventRules): void {
        ...
        eventRules.ruleServerRules.map((rule) =>
            this.ruleStopsByToken[this.currentToken].push(rule.id),
        );
    }
    ...
    private applyRuleChange(change: ListRuleChange): void {
        if (change.changeType === ChangeType.Added) {
            const eventRules = new EventRules(
                change.event.event_id,
                change.event.room_id,
                toRuleServerFormat(change.rule),
                this.currentToken,
            );
            this.addEventRules(eventRules);
        } else if (change.changeType === ChangeType.Modified) {
            const entry: EventRules | undefined = this.getEventRules(
                change.event.roomId,
            ...
            if (entry === undefined) {
                LogService.error(
                    "RuleServer",
                    `Could not find the rules for the previous modified state ${change.event["state_type"]} ${change.event["state_key"]} ${change.previousState?.event_id}`,
                );
                return;
            }
     ...
     */
    public getUpdates(sinceToken: string | null): {
        start: RuleServerRule[];
        stop: string[];
        reset?: boolean;
        since: string;
    } {
        const updatesSince = <T = EventRules | string>(
            token: number | null,
            policyStore: T[][],
        ): T[] => {
            if (token === null) {
        ...
        const [serverId, since] = sinceToken
            ? sinceToken.split("::")
            : [null, null];
        ...
        if (serverId && serverId !== this.serverId) {
            ...
            return {
                start: updatesSince(null, this.ruleStartsByToken)
                    .map((e: EventRules) => e.ruleServerRules)
                    .flat(),
            ...
            return {
                start: updatesSince(parsedSince, this.ruleStartsByToken)
                    .map((e: EventRules) => e.ruleServerRules)
                    .flat(),
...
function toRuleServerFormat(policyRule: ListRule): RuleServerRule[] {
    ...
    if (policyRule.kind === EntityType.RULE_USER) {
        ...
        LogService.info(
            "RuleServer",
            `Ignoring unsupported policy rule type ${policyRule.kind}`,
        );
        return [];
    }
}
