...
export class ReportManager extends EventEmitter {
    ...
    constructor(public mjolnir: Mjolnir) {
        ...
        mjolnir.matrixEmitter.on("room.event", async (roomId, event) => {
            try {
                ...
                LogService.error(
                    "ReportManager",
                    "Uncaught error while handling an event",
                    ex,
                );
            }
        });
    ...
    public async handleReaction({
        ...
        try {
            let originalEvent = await this.mjolnir.client.getEvent(
                roomId,
                relation.event_id,
            );
            if (
                originalEvent.sender !== (await this.mjolnir.client.getUserId())
            ) {
        ...
        */
        ...
        if (confirmationReport) {
            ...
            if (action.needsConfirmation) {
                ...
                let confirmation = {
                    msgtype: "m.notice",
                    body: `${action.emoji} ${await action.title(
                        this,
                        initialNoticeReport,
                    )}?`,
                    "m.relationship": {
                ...
                await this.mjolnir.client.sendEvent(
                    this.mjolnir.managementRoomId,
                    "m.reaction",
                    {
                        "m.relates_to": {
                            ...
                            key: `🆗 ${action.emoji} ${await action.title(
                                this,
                                initialNoticeReport,
...
class IgnoreBadReport implements IUIAction {
    ...
    public async execute(
        ...
        await manager.mjolnir.client.sendEvent(
            manager.mjolnir.managementRoomId,
            "m.room.message",
            {
                msgtype: "m.notice",
...
class RedactMessage implements IUIAction {
    ...
    public async execute(
        ...
        await manager.mjolnir.client.redactEvent(
            report.room_id,
            report.event_id,
        );
        return;
...
class KickAccused implements IUIAction {
    ...
    public async help(
        ...
        return `Kick ${htmlEscape(report.accused_id)} from room ${htmlEscape(
            report.room_alias_or_id,
        )}`;
    }
    public async execute(
        manager: ReportManager,
        ...
        await manager.mjolnir.client.kickUser(
            report.accused_id,
            report.room_id,
        );
        return;
...
class MuteAccused implements IUIAction {
    ...
    public async help(
        ...
        return `Mute ${htmlEscape(report.accused_id)} in room ${htmlEscape(
            report.room_alias_or_id,
        )}`;
    }
    public async execute(
        ...
        await manager.mjolnir.client.setUserPowerLevel(
            report.accused_id,
            report.room_id,
            -1,
        );
...
class BanAccused implements IUIAction {
    ...
    public async help(
        ...
        return `Ban ${htmlEscape(report.accused_id)} from room ${htmlEscape(
            report.room_alias_or_id,
        )}`;
    }
    public async execute(
    ...
    public async execute(
        ...
        for (let action of ACTION_LIST) {
            if (await action.canExecute(manager, report, moderationRoomId)) {
                list.push(
                    `<li>${action.emoji} ${await action.help(
                        manager,
                        report,
                    )}</li>`,
                );
        ...
        if (
            ...
            list.push(
                `<li>Some actions were disabled because Mjölnir is not moderator in room ${htmlEscape(
                    report.room_alias_or_id,
                )}</li>`,
            );
        }
...
class EscalateToServerModerationRoom implements IUIAction {
    ...
    public async canExecute(
        ...
        try {
            await manager.mjolnir.client.getEvent(
                report.room_id,
                report.event_id,
            );
        } catch (ex) {
    ...
    public async help(
        ...
        return `Escalate report to ${getHomeserver(
            await manager.mjolnir.client.getUserId(),
        )} server moderators`;
    }
    public async execute(
        ...
        let event = await manager.mjolnir.client.getEvent(
            report.room_id,
            report.event_id,
        );
...
class DisplayManager {
    ...
    public async displayReportAndUI(args: {
        ...
        try {
            roomAliasOrId =
                (await this.owner.mjolnir.client.getPublishedAlias(roomId)) ||
                roomId;
        } catch (ex) {
            // Ignore.
        }
        ...
        try {
            reporterDisplayName =
                (await this.owner.mjolnir.client.getUserProfile(reporterId))[
                    "displayname"
                ] || reporterId;
        } catch (ex) {
            reporterDisplayName =
                "<Error: Cannot extract reporter display name>";
        }
        try {
            accusedDisplayName =
                (await this.owner.mjolnir.client.getUserProfile(accusedId))[
                    "displayname"
                ] || accusedId;
        } catch (ex) {
        ...
        let eventShortcut = `https://matrix.to/#/${encodeURIComponent(
            roomId,
        )}/${encodeURIComponent(eventId)}`;
        let roomShortcut = `https://matrix.to/#/${encodeURIComponent(
            roomAliasOrId,
        )}`;
        ...
        let noticeEventId = await this.owner.mjolnir.client.sendMessage(
            this.owner.mjolnir.managementRoomId,
            notice,
        );
        if (kind !== Kind.ERROR) {
            ...
            for (let [label, action] of ACTIONS) {
                ...
                await this.owner.mjolnir.client.sendEvent(
                    this.owner.mjolnir.managementRoomId,
                    "m.reaction",
                    {
                        "m.relates_to": {
