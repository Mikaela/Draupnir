...
import {
    findPolicyListFromRoomReference,
    findPolicyListFromShortcode,
} from "./Ban";
        ...
        for (const member of members) {
            ...
            if (rule.test(victim)) {
                await mjolnir.managementRoomOutput.logMessage(
                    LogLevel.DEBUG,
                    "Unban",
                    `Unbanning ${victim} in ${protectedRoomId}`,
                    protectedRoomId,
                );
                ...
                if (!mjolnir.config.noop) {
                    ...
                    await mjolnir.managementRoomOutput.logMessage(
                        LogLevel.WARN,
                        "Unban",
                        `Attempted to unban ${victim} in ${protectedRoomId} but Mjolnir is running in no-op mode`,
                        protectedRoomId,
                    );
                }
...
async function unban(
    ...
    if (typeof entity === "string" || entity instanceof UserID) {
        const rawEnttiy =
            typeof entity === "string" ? entity : entity.toString();
