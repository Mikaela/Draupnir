...
function parseToken<T>(
    ...
    if ("error" in result) {
        if (result.error) {
            return {
                error: `Invalid ${name} ${htmlEscape(token)}: ${result.error}`,
            };
        } else {
...
export async function execSinceCommand(
    destinationRoomId: string,
    event: any,
    mjolnir: Mjolnir,
    tokens: ParseEntry[],
) {
    let result = await execSinceCommandAux(
        destinationRoomId,
        event,
        mjolnir,
        tokens,
    ...
    if ("error" in result) {
        mjolnir.client.unstableApis.addReactionToEvent(
            destinationRoomId,
            event["event_id"],
            "❌",
        );
        ...
        /* no need to await */ mjolnir.client.sendMessage(
            destinationRoomId,
            reply,
        );
    } else {
...
function formatResult(
    ...
    const html = `Attempted to ${action} ${
        recentJoins.length
    } users from room ${targetRoomId}.<br/>Succeeded ${
        summary.succeeded.length
    }: <ul>${summary.succeeded
        ...
        .map((x) => `<li>${htmlEscape(x)}</li>`)
        .join("\n")}</ul>`;
    const text = `Attempted to ${action} ${
        recentJoins.length
    } users from room ${targetRoomId}.\nSucceeded ${
        summary.succeeded.length
    }: ${summary.succeeded
        .map((x) => `*${htmlEscape(x)}`)
...
async function execSinceCommandAux(
    ...
    const minDateResult = parseToken(
        "<date>/<duration>",
        dateOrDurationToken,
        (source) => {
            ...
            if (!Number.isNaN(maybeMaxAgeMS)) {
                return {
                    ok: { minDate: maybeMinDate, maxAgeMS: maybeMaxAgeMS },
                };
            }
            ...
            if (maybeMaxAgeMS && !Number.isNaN(maybeMaxAgeMS)) {
                ...
                return {
                    ok: {
                        minDate: new Date(Date.now() - maybeMaxAgeMS),
                        maxAgeMS: maybeMaxAgeMS,
                    },
                };
            }
            return { error: "" };
    ...
    // Parse max entries.
    const maxEntriesResult = parseToken(
        "<maxEntries>",
        maxEntriesToken,
        (source) => {
    ...
    for (let token of optionalTokens) {
        const maybeArg = getTokenAsString(
            reasonParts ? "[reason]" : "[room]",
            token,
        );
    ...
    for (let targetRoomId of rooms) {
        let { html, text } = await (async () => {
            ...
            const recentJoins = mjolnir.roomJoins.getUsersInRoom(
                targetRoomId,
                minDate,
                maxEntries,
            );
            ...
            switch (action) {
                ...
                case Action.Kick: {
                    for (let join of recentJoins) {
                        try {
                            ...
                            LogService.warn(
                                "SinceCommand",
                                "Error while attempting to kick user",
                                ex,
                            );
                ...
                case Action.Ban: {
                    for (let join of recentJoins) {
                        try {
                            ...
                            LogService.warn(
                                "SinceCommand",
                                "Error while attempting to ban user",
                                ex,
                            );
                            results.failed.push(join.userId);
                        }
                ...
                case Action.Mute: {
                    ...
                    try {
                        ...
                        LogService.warn(
                            "SinceCommand",
                            "Error while attempting to mute users",
                            ex,
                        );
                        for (let join of recentJoins) {
                            results.failed.push(join.userId);
                ...
                case Action.Unmute: {
                    const powerLevels = (await mjolnir.client.getRoomStateEvent(
                        ...
                        users: Record</* userId */ string, number>;
                    ...
                    try {
                        ...
                        LogService.warn(
                            "SinceCommand",
                            "Error while attempting to unmute users",
                            ex,
                        );
                        for (let join of recentJoins) {
                            results.failed.push(join.userId);
