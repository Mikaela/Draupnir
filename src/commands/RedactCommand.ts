...
export async function execRedactCommand(
    roomId: string,
    event: any,
    mjolnir: Mjolnir,
    parts: string[],
    ...
    if (userId[0] !== "@") {
        ...
        if (
            parsed.roomIdOrAlias === undefined ||
            parsed.eventId === undefined
        ) {
            throw new TypeError(`Got a malformed permalink ${parsed}`);
        }
        const targetRoomId = await mjolnir.client.resolveRoom(
