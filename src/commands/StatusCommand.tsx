...
function mjolnirStatusInfo(this: MjolnirContext): StatusInfo {
    ...
    return {
        ...
        numberOfProtectedRooms:
            this.mjolnir.protectedRoomsTracker.getProtectedRooms().length,
        subscribedLists: this.mjolnir.policyListManager.lists
            .filter(
                (list) =>
                    !this.mjolnir.explicitlyProtectedRooms.includes(
                        list.roomId,
                    ),
            )
            .map(listInfo),
        subscribedAndProtectedLists: this.mjolnir.policyListManager.lists
            .filter((list) =>
                this.mjolnir.explicitlyProtectedRooms.includes(list.roomId),
            )
            .map(listInfo),
...
defineMatrixInterfaceAdaptor({
    ...
    renderer: async function (
        ...
        event: any,
        result: CommandResult<StatusInfo>,
    ): Promise<void> {
            ...
            const listInfo = lists.map((list) => {
                return (
                    <li>
                        ...
                        (rules: {list.serverRules} servers, {list.userRules}{" "}
                        users, {list.roomRules} rooms)
                    </li>
                );
            });
            return (
        ...
        await renderMatrixAndSend(
            <root>
                ...
                <br />
                {renderPolicyLists(
                    "Subscribed policy lists",
                    info.subscribedLists,
                )}
                {renderPolicyLists(
                    "Subscribed and protected policy lists",
...
defineInterfaceCommand({
    ...
    command: async function (
        ...
        CommandResult<Awaited<ReturnType<Protection["statusCommand"]>>>
    > {
        const protection =
            this.mjolnir.protectionManager.getProtection(protectionName);
        if (!protection) {
            return CommandError.Result(`Unknown protection ${protectionName}`);
        }
        return CommandResult.Ok(
            await protection.statusCommand(this.mjolnir, subcommands),
        );
    },
