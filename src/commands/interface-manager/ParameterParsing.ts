...
export interface IArgumentStream extends ISuperCoolStream<ReadItem[]> {
    ...
    ): Promise<CommandResult<ReadItem>>;
}

export class ArgumentStream
    extends SuperCoolStream<ReadItem[]>
    implements IArgumentStream
{
    public rest() {
    ...
    prompt(
        ...
        throw new TypeError(
            "This argument stream is NOT promptable, did you even check isPromptable().",
        );
    }
}
...
export function simpleTypeValidator(
    name: string,
    predicate: (readItem: ReadItem) => boolean,
): PredicateIsParameter {
    return (readItem: ReadItem) => {
        ...
        if (result) {
            ...
            return CommandError.Result(
                `Was expecting a match for the presentation type: ${name} but got ${readItem}.`,
            );
        }
    };
}

export function presentationTypeOf(
    presentation: unknown,
): PresentationType | undefined {
    // We have no concept of presentation-subtype
    // But we have a top type which is any...
    const candidates = [...PRESENTATION_TYPES.values()].filter(
        (possibleType) =>
            possibleType.validator(presentation as ReadItem).isOk() &&
    ...
    if (candidates.length === 0) {
        ...
        throw new TypeError(
            `presentationTypeof: There are multiple candidates for the presentation ${presentation}: ${JSON.stringify(
...
makePresentationType({
    ...
    validator: simpleTypeValidator(
        "Keyword",
        (item: ReadItem) => item instanceof Keyword,
    ),
});
...
makePresentationType({
    ...
    validator: simpleTypeValidator(
        "string",
        (item: ReadItem) => typeof item === "string",
    ),
});

makePresentationType({
    name: "boolean",
    validator: simpleTypeValidator("boolean", (item: ReadItem) =>
        item === "true" ? true : item === "false",
    ),
});
 ...
 */
export class RestDescription<ExecutorContext = unknown>
    ...
    public async parseRest(
        ...
        while (stream.peekItem() !== undefined) {
            ...
            if (stream.peekItem() !== undefined) {
                const validationResult = this.acceptor.validator(
                    stream.peekItem(),
                );
...
export class ParsedKeywords {
    ...
    public getKeyword<T extends ReadItem | boolean>(
        keyword: string,
        ...
        if (keywordDescription === undefined) {
            throw new TypeError(
                `${keyword} is not a keyword that has been expected for this command.`,
            );
        }
...
class KeywordParser {
    ...
    private readKeywordAssociatedProperty(
        ...
        itemStream: IArgumentStream,
    ): CommandResult<any, ArgumentParseError> {
        if (
            itemStream.peekItem() !== undefined &&
            !(itemStream.peekItem() instanceof Keyword)
        ) {
            const validationResult = keyword.acceptor.validator(
                itemStream.peekItem(),
            );
            if (validationResult.isOk()) {
                return CommandResult.Ok(itemStream.readItem());
            } else {
                return ArgumentParseError.Result(validationResult.err.message, {
                    parameter: keyword,
                    stream: itemStream,
                });
            }
            ...
            if (!keyword.isFlag) {
                return ArgumentParseError.Result(
                    ...
                    { parameter: keyword, stream: itemStream },
                );
            } else {
    ...
    public parseKeywords(itemStream: IArgumentStream): CommandResult<this> {
        while (
            ...
            if (description === undefined) {
                ...
                const associatedPropertyResult =
                    this.readKeywordAssociatedProperty(description, itemStream);
                if (associatedPropertyResult.isErr()) {
                    return associatedPropertyResult;
                } else {
                    this.arguments.set(
                        description.name,
                        associatedPropertyResult.ok,
                    );
                }
            }
        }
    ...
    ): Promise<CommandResult<ReadItem[] | undefined>> {
        if (restDescription !== undefined) {
            return await restDescription.parseRest(
                stream,
                shouldPromptForRest,
                this,
            );
            ...
            if (stream.peekItem() !== undefined) {
                return CommandError.Result(
                    `There is an unexpected non-keyword argument ${JSON.stringify(
                        stream.peekItem(),
                    )}`,
                );
            } else {
...
export type Prompt<ExecutorContext> = (
...
export type ParameterParser = (
    stream: IArgumentStream,
) => Promise<CommandResult<ParsedArguments>>;
...
export function parameters(
    ...
    rest: undefined | RestDescription = undefined,
    keywords: KeywordsDescription = new KeywordsDescription({}, false),
): IArgumentListParser {
...
class ArgumentListParser implements IArgumentListParser {
    ...
    public async parse(
        ...
        for (const parameter of this.descriptions) {
            ...
            if (result.isErr()) {
                return ArgumentParseError.Result(result.err.message, {
                    parameter,
                    stream,
                });
            }
        ...
        const restResult = await keywordsParser.parseRest(
            stream,
            hasPrompted,
            this.rest,
        );
        ...
        const immediateArguments =
            restResult.ok === undefined || restResult.ok.length === 0
                ? stream.source
                : stream.source.slice(
                      0,
...
export class AbstractArgumentParseError extends CommandError {
    ...
    public static Result<Ok>(
        message: string,
        options: { stream: IArgumentStream },
    ): CommandResult<Ok, AbstractArgumentParseError> {
        return CommandResult.Err(
            new AbstractArgumentParseError(options.stream, message),
        );
    }
}
...
export class ArgumentParseError extends AbstractArgumentParseError {
    ...
    public static Result<Ok>(
        message: string,
        options: { parameter: ParameterDescription; stream: IArgumentStream },
    ): CommandResult<Ok, ArgumentParseError> {
        return CommandResult.Err(
...
export class UnexpectedArgumentError extends AbstractArgumentParseError {
    public static Result<Ok>(
        message: string,
        options: { stream: IArgumentStream },
    ): CommandResult<Ok, UnexpectedArgumentError> {
        return CommandResult.Err(
            new UnexpectedArgumentError(options.stream, message),
        );
    }
}
    ...
    return {
        ...
        validator: (readItem: ReadItem) => {
            if (presentationTypes.some((p) => p.validator(readItem).isOk())) {
                ...
                return CommandError.Result(
                    `Read item didn't match any of the presentaiton types ${name}`,
                );
            }
        },
    };
