...
const PRESENTATION_RENDERERS = new Map<
    PresentationType,
    PresentationRenderer
>();

// don't forget that this is the equivalent of present and define-presentation-method.
export function definePresentationRenderer(
    presentationType: PresentationType,
    renderer: PresentationRenderer,
): void {
    if (PRESENTATION_RENDERERS.has(presentationType)) {
        throw new TypeError(
            `A DeadDocument renderer is already defined for the presentation type ${presentationType.name}`,
        );
    } else {
...
export function findPresentationRenderer(
    presentationType: PresentationType,
): PresentationRenderer {
    const entry = PRESENTATION_RENDERERS.get(presentationType);
    if (entry === undefined) {
        throw new TypeError(
            `There is no presentation renderer defined for the presentation type ${presentationType.name}`,
        );
    }
    return entry;
}
