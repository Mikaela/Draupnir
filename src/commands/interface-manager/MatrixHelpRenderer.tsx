...
export async function renderHelp(
    ...
    event: any,
    result: CommandResult<InterfaceCommand<BaseFunction>[], CommandError>,
): Promise<void> {
...
export async function tickCrossRenderer(
    this: MatrixInterfaceAdaptor<MatrixContext, BaseFunction>,
    client: MatrixSendClient,
    commandRoomId: string,
    event: any,
    result: CommandResult<unknown, CommandError>,
): Promise<void> {
...
function formattedArgumentHint(
    ...
    let badArgument = ` ${error.stream.peekItem()}\n${Array(
        commandContext.length + 1,
    ).join(" ")} ^ expected ${error.parameter.acceptor.name} here`;
...
function renderArgumentParseError(
    ...
    return (
        <root>
            ...
            <code>{error.parameter.name}</code> parameter for this command.
            <br />
