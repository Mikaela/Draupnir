...
export type MjolnirBaseExecutor = (
    this: MjolnirContext,
    ...args: any[]
...
export async function handleCommand(
    ...
    const tokens = tokenize(cmd.replace("#", "\\#")).slice(
        /* get rid of ["!mjolnir", command] */ 2,
    );

    try {
        if (parts[1] === "joins") {
            return await showJoinsStatus(
                roomId,
                event,
                mjolnir,
                parts.slice(/* ["joins"] */ 2),
            ...
            return await execSetDefaultListCommand(
                roomId,
                event,
                mjolnir,
                parts,
            ...
            return await execConfigSetProtection(
                roomId,
                event,
                mjolnir,
                parts.slice(3),
            ...
            return await execConfigAddProtection(
                roomId,
                event,
                mjolnir,
                parts.slice(3),
            ...
            return await execConfigRemoveProtection(
                roomId,
                event,
                mjolnir,
                parts.slice(3),
            ...
            return await execConfigGetProtection(
                roomId,
                event,
                mjolnir,
                parts.slice(3),
            ...
            return await execSetPowerLevelCommand(
                roomId,
                event,
                mjolnir,
                parts,
            ...
            const mjolnirContext: MjolnirContext = {
                ...
                roomId,
                event,
                client: mjolnir.client,
                emitter: mjolnir.matrixEmitter,
            };
            try {
                return await adaptor.invoke(
                    mjolnirContext,
                    mjolnirContext,
                    ...stream.rest(),
                );
            } catch (e) {
                const commandError = new CommandException(
                    e,
