...
export async function execImportCommand(
    roomId: string,
    event: any,
    mjolnir: Mjolnir,
    parts: string[],
    ...
    const list = mjolnir.policyListManager.lists.find(
        (b) => b.listShortcode === parts[3],
    ) as PolicyList;
    if (!list) {
        const errMessage = "Unable to find list - check your shortcode.";
        const errReply = RichReply.createFor(
            roomId,
            event,
            errMessage,
            errMessage,
    ...
    for (const stateEvent of state) {
        ...
        if (
            ...
            for (const server of content["deny"]) {
                ...
                await mjolnir.client.sendNotice(
                    mjolnir.managementRoomId,
                    `Adding server ${server} to ban list`,
                );
