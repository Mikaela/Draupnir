...
class RoomMembers {
    ...
    private _joinsByUser: Map<string /* user id */, number /* timestamp */> =
        new Map();
     ...
     * the oldest.
     */
    private _leaves: Map<string /* user id */, number /* timestamp */> =
        new Map();
    ...
    public cleanup() {
        ...
        this._joinsByTimestamp = this._joinsByTimestamp.filter((join) =>
            this.isStillValid(join),
        );
...
export class RoomMemberManager {
    ...
    public async handleEvent(roomId: string, event: any, now?: Date) {
        ...
        if (userState === "join" && prevMembership !== "join") {
            ...
            LEAVE_OR_BAN.includes(userState) &&
            !LEAVE_OR_BAN.includes(prevMembership)
        ) {
            action = Action.Leave;
        } else {
            action = Action.Other;
