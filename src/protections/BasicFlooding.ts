...
export class BasicFlooding extends Protection {
    private lastEvents: {
        [roomId: string]: {
            [userId: string]: { originServerTs: number; eventId: string }[];
        };
    } = {};
    ...
    public get description(): string {
        return (
            "If a user posts more than " +
            DEFAULT_MAX_PER_MINUTE +
            " messages in 60s they'll be " +
    ...
    public async handleEvent(
        ...
        if (messageCount >= this.settings.maxPerMinute.value) {
            ...
            if (!mjolnir.config.noop) {
                ...
                await mjolnir.managementRoomOutput.logMessage(
                    LogLevel.WARN,
                    "BasicFlooding",
                    `Tried to redact messages for ${event["sender"]} in ${roomId} but Mjolnir is running in no-op mode`,
                    roomId,
                );
            }
        ...
        if (forUser.length > this.settings.maxPerMinute.value * 2) {
            forUser.splice(
                0,
                forUser.length - this.settings.maxPerMinute.value * 2 - 1,
            );
        }
    }
}
