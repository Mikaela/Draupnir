...
export class WordList extends Protection {
    ...
    public async handleEvent(
        ...
        const minsBeforeTrusting =
            mjolnir.config.protections.wordlist.minutesBeforeTrusting;
            ...
            if (event["type"] === "m.room.member") {
                if (isTrueJoinEvent(event)) {
                    ...
                    LogService.info(
                        ...
                        `${
                            event["state_key"]
                        } joined ${roomId} at ${now.toDateString()}`,
                    );
                } else if (
        ...
        if (event["type"] === "m.room.message") {
            const message =
                content["formatted_body"] || content["body"] || null;
            if (!message) {
                return;
            }
            ...
            if (match) {
                ...
                return [
                    new ConsequenceBan(reason),
                    new ConsequenceRedact(reason),
                ];
            }
        }
    }
