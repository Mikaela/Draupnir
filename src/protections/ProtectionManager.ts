...
class EnabledProtectionsManager extends MatrixDataManager<EnabledProtectionsEvent> {
    protected readonly schema: SchemaMigration[] = [
        async function enableBanPropagationByDefault(
            input: EnabledProtectionsEvent,
        ) {
            const enabled = new Set(input.enabled);
            const banPropagationProtection = PROTECTIONS.find(
                (p) => p.name === "BanPropagationProtection",
            );
            if (banPropagationProtection === undefined) {
                throw new TypeError(
                    "Couldn't find the ban propagation protection",
                );
            }
    ...
    private readonly enabledProtections =
        new Set</* protection name */ string>();
    ...
    protected async requestMatrixData(): Promise<unknown> {
        try {
            return await this.mjolnir.client.getAccountData(
                ENABLED_PROTECTIONS_EVENT_TYPE,
            );
        } catch (e) {
            if (e.statusCode === 404) {
                LogService.warn(
                    "PolicyListManager",
                    "Couldn't find account data for Draupnir's protections, assuming first start.",
                    e,
                );
...
export class ProtectionManager {
    ...
    constructor(private readonly mjolnir: Mjolnir) {
        this.enabledProtectionsManager = new EnabledProtectionsManager(
            this.mjolnir,
        );
    }
    ...
    public async start() {
        ...
        this.mjolnir.reportManager.on(
            "report.new",
            this.handleReport.bind(this),
        );
        this.mjolnir.matrixEmitter.on(
            "room.event",
        ...
        for (const protection of PROTECTIONS) {
            try {
                ...
                LogService.error(
                    "ProtectionManager",
                    `Unable to start protection ${protection.name}`,
                    e,
                );
    ...
    public async registerProtection(protection: Protection) {
        ...
        protection.enabled =
            this.enabledProtectionsManager.isEnabled(protection) ?? false;
    ...
    public unregisterProtection(protectionName: string) {
        if (!this._protections.has(protectionName)) {
            throw new Error(
                "Failed to find protection by name: " + protectionName,
            );
        }
     ...
     */
    public async setProtectionSettings(
        protectionName: string,
        changedSettings: { [setting: string]: any },
    ): Promise<any> {
        ...
        for (let [key, value] of Object.entries(changedSettings)) {
            if (!(key in protection.settings)) {
                throw new ProtectionSettingValidationError(
                    `Failed to find protection setting by name: ${key}`,
                );
            }
            if (typeof protection.settings[key].value !== typeof value) {
                throw new ProtectionSettingValidationError(
                    `Invalid type for protection setting: ${key} (${typeof value})`,
                );
            }
            if (!protection.settings[key].validate(value)) {
                throw new ProtectionSettingValidationError(
                    `Invalid value for protection setting: ${key} (${value})`,
                );
            }
    ...
    public async getProtectionSettings(
        ...
        const settingDefinitions =
            this._protections.get(protectionName)?.settings ?? {};
        const validatedSettings: { [setting: string]: any } = {};
        for (let [key, value] of Object.entries(savedSettings)) {
            if (
        ...
        for (const consequence of consequences) {
            try {
                if (consequence.name === "alert") {
                    ...
                    await this.mjolnir.client.banUser(
                        sender,
                        roomId,
                        "abuse detected",
                    );
                } else if (consequence.name === "redact") {
                    await this.mjolnir.client.redactEvent(
                        roomId,
                        eventId,
                        "abuse detected",
                    );
    ...
    private async handleEvent(roomId: string, event: any) {
        if (
            this.mjolnir.protectedRoomsTracker
                .getProtectedRooms()
                .includes(roomId)
        ) {
            ...
            for (const protection of this.enabledProtections) {
                ...
                try {
                    consequences = await protection.handleEvent(
                        this.mjolnir,
                        roomId,
                        event,
                    );
                } catch (e) {
                    const eventPermalink = Permalinks.forEvent(
                        roomId,
                        event["event_id"],
                    );
                    LogService.error(
                        "ProtectionManager",
                        "Error handling protection: " + protection.name,
                    );
                    LogService.error(
                        "ProtectionManager",
                        "Failed event: " + eventPermalink,
                    );
            ...
            await this.mjolnir.unlistedUserRedactionHandler.handleEvent(
                roomId,
                event,
                this.mjolnir,
            ); // FIXME: That's rather spaghetti
        }
    }
    ...
    private requiredProtectionPermissions(): Set<string> {
        return new Set(
            this.enabledProtections
                .map((p) => p.requiredStatePermissions)
                .flat(),
        );
    }

    public async verifyPermissionsIn(
        roomId: string,
        ...
        try {
            ...
            const powerLevels = await this.mjolnir.client.getRoomStateEvent(
                roomId,
                "m.room.power_levels",
                "",
            );
            ...
            const aclLevel = plDefault(
                events["m.room.server_acl"],
                stateDefault,
            );
            ...
            for (const additionalPermission of additionalPermissions) {
                const permLevel = plDefault(
                    events[additionalPermission],
                    stateDefault,
                );
