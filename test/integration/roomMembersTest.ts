...
describe("Test: Testing RoomMemberManager", function () {
    it("RoomMemberManager counts correctly when we call handleEvent manually", async function () {
        ...
        const ROOMS = ["!room_0@localhost", "!room_1@localhost"];
...

        {
            ...
            const joins0ByUserId = new Map(
                joins0.map((join) => [join.userId, join.timestamp]),
            );
            const joins1ByUserId = new Map(
                joins1.map((join) => [join.userId, join.timestamp]),
            );
            ...
            for (let i = 0; i < SAMPLE_SIZE; ++i) {
                ...
                assert.ok(
                    ts,
                    `User ${user} should have been seen joining room ${i % 2}`,
                );
                assert.equal(
                    ts,
                    joinDate(i).getTime(),
            ...
            assert.equal(
                joins0ByUserId.size,
                0,
                "We should have found all the users in room 0",
            );
            assert.equal(
                joins1ByUserId.size,
                0,
                "We should have found all the users in room 1",
            );
        }

        // Now, let's add a few leave events.
        let leaveDate = (i: number) =>
            new Date(start.getTime() + (SAMPLE_SIZE + i) * 100_000);
        ...
        // Let's see if we have properly updated the joins/leaves
        {
            ...
            for (let i = 0; i < SAMPLE_SIZE; ++i) {
                ...
                if (isStillJoined) {
                    assert.ok(
                        ts,
                        `User ${user} should have been seen joining room ${
                            i % 2
                        }`,
                    );
                    assert.equal(
                    ...
                    assert.ok(
                        !ts,
                        `User ${user} should not be seen as a member of room ${
                            i % 2
                        } anymore`,
                    );
                }
        ...
        let rejoinDate = (i: number) =>
            new Date(start.getTime() + (SAMPLE_SIZE * 2 + i) * 100_000);
        ...
        // Let's see if we have properly updated the joins/leaves
        {
            ...
            for (let i = 0; i < SAMPLE_SIZE; ++i) {
                ...
                if (hasRejoined) {
                    assert.ok(
                        ts,
                        `User ${user} should have been seen rejoining room ${
                            i % 2
                        }`,
                    );
                    assert.equal(
        ...
        {
            const joins0 = manager.getUsersInRoom(
                ROOMS[0],
                rejoinDate(-1),
                100_000,
            );
            const joins1 = manager.getUsersInRoom(
                ROOMS[1],
                rejoinDate(-1),
                100_000,
            );
            ...
            for (let i = 0; i < SAMPLE_SIZE; ++i) {
                ...
                if (hasRejoined) {
                    ...
                    assert.ok(
                        !ts,
                        `When looking only at recent entries, user ${user} should not be seen as a member of room ${
                            i % 2
                        } anymore`,
                    );
                }
            ...
            assert.equal(
                joins0ByUserId.size,
                0,
                "We should have found all the users who recently joined room 0",
            );
            assert.equal(
                joins1ByUserId.size,
                0,
                "We should have found all the users who recently joined room 1",
            );
        }
        ...
        // Let's see if we have properly updated the joins/leaves
        {
            ...
            for (let i = 0; i < SAMPLE_SIZE; ++i) {
                ...
                if (hasRejoined) {
                    assert.ok(
                        ts,
                        `After cleanup, user ${user} should have been seen rejoining room ${
                            i % 2
                        }`,
                    );
                    assert.equal(
                    ...
                    assert.ok(
                        !ts,
                        `After cleanup, user ${user} should not be seen as a member of room ${
                            i % 2
                        } anymore`,
                    );
                } else {
                    assert.ok(
                        ts,
                        `After cleanup, user ${user} should have been seen joining room ${
                            i % 2
                        }`,
                    );
                    assert.equal(
            ...
            assert.equal(
                joins0ByUserId.size,
                0,
                "After cleanup, we should have found all the users in room 0",
            );
            assert.equal(
                joins1ByUserId.size,
                0,
                "After cleanup, we should have found all the users in room 1",
            );
        }
        ...
        // Now let's check only the most recent joins.
        {
            ...
            for (let i = 0; i < SAMPLE_SIZE; ++i) {
                ...
                if (hasRejoined) {
                    ...
                    assert.ok(
                        !ts,
                        `After cleanup, when looking only at recent entries, user ${user} should not be seen as a member of room ${
                            i % 2
                        } anymore`,
                    );
                }
    ...
    it("RoomMemberManager counts correctly when we actually join/leave/get banned from the room", async function () {
        ...
        // Setup a moderator.
        this.moderator = await newTestUser(this.config.homeserverUrl, {
            name: { contains: "moderator" },
        });
        await mjolnir.client.inviteUser(
            await this.moderator.getUserId(),
        ...
        for (let i = 0; i < SAMPLE_SIZE; ++i) {
            this.users.push(
                await newTestUser(this.config.homeserverUrl, {
                    name: { contains: `user_${i}_room_member_test` },
                }),
            );
        }
        const userIds = [];
        ...
        for (let roomId of roomIds) {
            await this.moderator.sendMessage(mjolnir.managementRoomId, {
                msgtype: "m.text",
                body: `!mjolnir rooms add ${roomId}`,
            });
        }
        ...
        for (let roomId of roomIds) {
            ...
            assert.equal(
                joined.length,
                1,
                "Initially, we shouldn't know about any other user in these rooms",
            );
            assert.equal(
                joined[0].userId,
        ...
        for (let roomId of roomIds) {
            const reply = await getFirstReply(
                mjolnir.matrixEmitter,
                mjolnir.managementRoomId,
                () => {
                    const command = `!mjolnir joins ${roomId}`;
                    return this.moderator.sendMessage(
                        mjolnir.managementRoomId,
            ...
            assert.ok(
                body.includes("\n1 recent joins"),
                "Initially the command should respond with 1 user",
            );
        }
    ...
    it("!mjolnir since kicks the correct users", async function () {
        ...
        for (let i = 0; i < SAMPLE_SIZE; ++i) {
            this.goodUsers.push(
                await newTestUser(this.config.homeserverUrl, {
                    name: { contains: `good_user_${i}_room_member_test` },
                }),
            );
            this.badUsers.push(
                await newTestUser(this.config.homeserverUrl, {
                    name: { contains: `bad_user_${i}_room_member_test` },
                }),
            );
        }
        ...
        class Experiment {
            ...
            constructor({
                ...
                this.name = name;
                this.shouldAffectControlProtected =
                    typeof shouldAffectControlProtected === "undefined"
                        ? false
                        : shouldAffectControlProtected;
                this.command = command;
                this.n = typeof n === "undefined" ? 1 : n;
                this.method = method;
                this.isSameRoomAsPrevious =
                    typeof sameRoom === "undefined" ? false : sameRoom;
            }
            ...
            addTo(experiments: Experiment[]) {
                if (this.isSameRoomAsPrevious) {
                    this.roomIndex =
                        experiments[experiments.length - 1].roomIndex;
                } else if (experiments.length === 0) {
                    this.roomIndex = 0;
                } else {
                    this.roomIndex =
                        experiments[experiments.length - 1].roomIndex! + 1;
                }
        ...
        for (let experiment of [
            ...
            new Experiment({
                name: "kick with date",
                command: (roomId: string) =>
                    `!mjolnir since "${cutDate}" kick 100 ${roomId}`,
                method: Method.kick,
            }),
            ...
            new Experiment({
                name: "ban with date",
                command: (roomId: string) =>
                    `!mjolnir since "${cutDate}" ban 100 ${roomId}`,
                method: Method.ban,
            }),
            ...
            new Experiment({
                name: "mute with date",
                command: (roomId: string) =>
                    `!mjolnir since "${cutDate}" mute 100 ${roomId}`,
                method: Method.mute,
            }),
            new Experiment({
                name: "unmute with date",
                command: (roomId: string) =>
                    `!mjolnir since "${cutDate}" unmute 100 ${roomId}`,
                method: Method.unmute,
                sameRoom: true,
            }),
            ...
            new Experiment({
                ...
                command: (roomId: string) =>
                    `!mjolnir since ${
                        Date.now() - cutDate.getTime()
                    }ms kick 100 ${roomId} bad, bad user`,
                method: Method.kick,
            }),
            ...
            new Experiment({
                ...
                command: (roomId: string) =>
                    `!mjolnir since ${
                        Date.now() - cutDate.getTime()
                    }ms ban 100 ${roomId} bad, bad user`,
                method: Method.ban,
            }),
            ...
            new Experiment({
                ...
                command: (roomId: string) =>
                    `!mjolnir since ${
                        Date.now() - cutDate.getTime()
                    }ms mute 100 ${roomId} bad, bad user`,
                method: Method.mute,
            }),
            new Experiment({
                ...
                command: (roomId: string) =>
                    `!mjolnir since ${
                        Date.now() - cutDate.getTime()
                    }ms unmute 100 ${roomId} bad, bad user`,
                method: Method.unmute,
                sameRoom: true,
            }),
            ...
            new Experiment({
                ...
                command: (roomId: string) =>
                    `!mjolnir since "${cutDate}" kick 100 ${roomId} bad, bad user`,
            ...
            new Experiment({
                ...
                command: (roomId: string) =>
                    `!mjolnir since "${cutDate}" ban 100 ${roomId} bad, bad user`,
                method: Method.ban,
            }),
            ...
            new Experiment({
                ...
                command: (roomId: string) =>
                    `!mjolnir since "${cutDate}" mute 100 ${roomId} bad, bad user`,
                method: Method.mute,
            }),
            new Experiment({
                name: "unmute with date and reason",
                command: (roomId: string) =>
                    `!mjolnir since "${cutDate}" unmute 100 ${roomId} bad, bad user`,
                method: Method.unmute,
                sameRoom: true,
            }),
            ...
            new Experiment({
                ...
                command: (_: string, roomAlias: string) =>
                    `!mjolnir since ${
                        Date.now() - cutDate.getTime()
                    }ms kick 100 ${roomAlias}`,
                method: Method.kick,
            }),
            ...
            new Experiment({
                ...
                command: (_: string, roomAlias: string) =>
                    `!mjolnir since ${
                        Date.now() - cutDate.getTime()
                    }ms kick 100 ${roomAlias} for some reason`,
                method: Method.kick,
            }),
            ...
            new Experiment({
                ...
                command: () =>
                    `!mjolnir since "${cutDate}" kick 100 * bad, bad user`,
        ...
        {
            const usersInUnprotectedControlProtected =
                await mjolnir.client.getJoinedRoomMembers(
                    CONTROL_UNPROTECTED_ROOM_ID,
                );
            const usersInControlProtected =
                await mjolnir.client.getJoinedRoomMembers(CONTROL_PROTECTED_ID);
        ...
        for (let i = 0; i < EXPERIMENTS.length; ++i) {
            ...
            console.debug(
                `Running experiment ${i} "${
                    experiment.name
                }" in room index ${index} (${roomId} / ${roomAlias}): \`${experiment.command(
                    roomId,
                    roomAlias,
                )}\``,
            );
            assert.ok(
                ...
                `In experiment ${experiment.name}, we should have seen ${
                    2 * SAMPLE_SIZE
                } users, saw ${joined.length}`,
            );
            ...
            await getNthReply(
                ...
                async () => {
                    ...
                    let result = await this.moderator.sendMessage(
                        mjolnir.managementRoomId,
                        { msgtype: "m.text", body: command },
                    );
                    return result;
            ...
            const usersInRoom =
                await mjolnir.client.getJoinedRoomMembers(roomId);
            const usersInUnprotectedControlProtected =
                await mjolnir.client.getJoinedRoomMembers(
                    CONTROL_UNPROTECTED_ROOM_ID,
                );
            ...
            if (experiment.method === Method.mute) {
                for (let userId of goodUserIds) {
                    let canSpeak = await mjolnir.client.userHasPowerLevelFor(
                        userId,
                        roomId,
                        "m.message",
                        false,
                ...
                for (let userId of badUserIds) {
                    ...
                    const leaveEvent = await mjolnir.client.getRoomStateEvent(
                        roomId,
                        "m.room.member",
                        userId,
                    );
