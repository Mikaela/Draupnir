...
async function getProtectedRoomsFromAccountData(
    client: MatrixSendClient,
): Promise<string[]> {
    const rooms: { rooms?: string[] } = await client.getAccountData(
...
describe("Test: config.protectAllJoinedRooms behaves correctly.", function () {
    it("does not clobber the account data.", async function () {
        ...
        const implicitlyProtectedRooms = await Promise.all(
            [...Array(2).keys()].map((_) =>
                moderator.createRoom({ preset: "public_chat" }),
            ),
        );
        await Promise.all(
            implicitlyProtectedRooms.map((roomId) =>
                mjolnir.client.joinRoom(roomId),
            ),
        );

        // we sync and check that none of them end up in account data
        await mjolnir.protectedRoomsTracker.syncLists();
        (await getProtectedRoomsFromAccountData(mjolnir.client)).forEach(
            (roomId) =>
        ...
        mjolnir.protectedRoomsTracker
            .getProtectedRooms()
            .forEach((roomId) =>
                assert.equal(implicitlyProtectedRooms.includes(roomId), true),
            );

        // We create one policy list with Mjolnir, and we watch another that is maintained by someone else.
        const policyListShortcode = await createBanList(
        ...
        await mjolnir.policyListManager.watchList(
            MatrixRoomReference.fromPermalink(unprotectedWatchedList.roomRef),
        );
        await mjolnir.protectedRoomsTracker.syncLists();
        ...
        assert.equal(
            mjolnir.protectedRoomsTracker
                .getProtectedRooms()
                .includes(unprotectedWatchedList.roomId),
            false,
        );
        const accountDataAfterListSetup =
            await getProtectedRoomsFromAccountData(mjolnir.client);
        assert.equal(
            accountDataAfterListSetup.includes(unprotectedWatchedList.roomId),
            false,
        );
