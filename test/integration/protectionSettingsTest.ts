...
describe("Test: Protection settings", function () {
    ...
    this.beforeEach(async function () {
        syncingUser = await newTestUser(this.config.homeserverUrl, {
    ...
    it("Mjolnir refuses to save invalid protection setting values", async function () {
        ...
        await assert.rejects(
            async () =>
                await this.mjolnir.protectionManager.setProtectionSettings(
    ...
    it("Mjolnir successfully saves valid protection setting values", async function () {
        ...
        await this.mjolnir.protectionManager.setProtectionSettings("05OVMS", {
            test: 123,
        });
        assert.equal(
            (
                await this.mjolnir.protectionManager.getProtectionSettings(
                    "05OVMS",
                )
            )["test"],
            123,
    ...
    it("Mjolnir should accumulate changed settings", async function () {
        ...
        await this.mjolnir.protectionManager.setProtectionSettings("HPUjKN", {
            test1: 1,
        });
        await this.mjolnir.protectionManager.setProtectionSettings("HPUjKN", {
            test2: 2,
        ...
        let reply = () =>
            new Promise((resolve, reject) => {
                client.on(
                    ...
                    noticeListener(this.mjolnir.managementRoomId, (event) => {
                        if (
                            event.content.body.includes("Changed oXzT0E.test ")
                        ) {
                            resolve(event);
                        }
                    }),
