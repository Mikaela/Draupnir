...
import {
    ConsequenceBan,
    ConsequenceRedact,
} from "../../src/protections/consequence";

describe("Test: standard consequences", function () {
    let badUser;
    let goodUser;
    this.beforeEach(async function () {
        badUser = await newTestUser(this.config.homeserverUrl, {
            name: { contains: "standard-consequences" },
        });
        goodUser = await newTestUser(this.config.homeserverUrl, {
            name: { contains: "standard-consequences" },
        });
        await badUser.start();
    ...
    it("Mjolnir applies a standard consequence redaction", async function () {
        ...
        let reply = new Promise(async (resolve, reject) => {
            const messageId = await badUser.sendMessage(protectedRoomId, {
                msgtype: "m.text",
                body: "ngmWkF",
            });
            let redaction;
            badUser.on("room.event", (roomId, event) => {
                ...
                if (
                    ...
                    event?.content?.body?.startsWith(
                        "protection JY2TPN enacting redact against ",
                    ) &&
                    redaction !== undefined
                ) {
        ...
        let reply = new Promise(async (resolve, reject) => {
            ...
            badUser.on("room.event", (roomId, event) => {
                if (
                    ...
                    event?.type === "m.room.message" &&
                    event?.content?.body?.startsWith(
                        "protection 0LxMTy enacting ban against ",
                    ) &&
                    ban !== undefined
                ) {
                    resolve([ban, event]);
    ...
    it("Mjolnir doesn't ban a good user", async function () {
        ...
        let reply = new Promise(async (resolve, reject) => {
            this.mjolnir.client.on("room.message", async (roomId, event) => {
                if (event?.content?.body === "SUwvFT") {
                    await badUser.sendMessage(protectedRoomId, {
                        msgtype: "m.text",
                        body: "8HUnwb",
                    });
                }
        ...
        await goodUser.sendMessage(protectedRoomId, {
            msgtype: "m.text",
            body: "SUwvFT",
        });
