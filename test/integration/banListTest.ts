...
import {
    ...
    RULE_USER,
    SERVER_RULE_TYPES,
} from "../../src/models/ListRule";
import AccessControlUnit, {
    Access,
    EntityAccess,
} from "../../src/models/AccessControlUnit";
...
describe("Test: Updating the PolicyList", function () {
    it("Calculates what has changed correctly.", async function () {
        ...
        const banListId = await mjolnir.client.createRoom({
            invite: [await moderator.getUserId()],
        });
        const banList = new PolicyList(banListId, banListId, mjolnir.client);
        await mjolnir.client.setUserPowerLevel(
        ...
        let originalEventId = await createPolicyRule(
            mjolnir.client,
            banListId,
            RULE_USER,
        ...
        let modifyingEventId = await createPolicyRule(
            mjolnir.client,
            banListId,
            RULE_USER,
            "@modified:localhost:9999",
        ...
        let modifyingAgainEventId = await createPolicyRule(
            mjolnir.client,
            banListId,
            RULE_USER,
            "@modified:localhost:9999",
        ...
        const redactThis = await createPolicyRule(
            mjolnir.client,
            banListId,
            RULE_USER,
        ...
        await banList.updateList();
        assert.equal(
            banList.userRules.filter(
                (r) => r.entity === "@redacted:localhost:9999",
            ).length,
            1,
        );
        ...
        assert.notEqual(
            Object.keys(changes[0].previousState["content"]),
            0,
            "Should have a copy of the unredacted state",
        );
        assert.notEqual(
            changes[0].rule,
            undefined,
            "The previous rule should be present",
        );
        assert.equal(
            banList.userRules.filter(
                (r) => r.entity === "@redacted:localhost:9999",
        ...
        await banList.updateList();
        assert.equal(
            banList.userRules.filter((r) => r.entity === softRedactedEntity)
                .length,
            1,
        );
        ...
        originalEventId = await createPolicyRule(
            mjolnir.client,
            banListId,
        ...
        assert.equal(changes[0].changeType, ChangeType.Added);
        assert.equal(
            banList.userRules.filter((r) => r.entity === "@old:localhost:9999")
                .length,
            1,
        );
        modifyingEventId = await createPolicyRule(
            mjolnir.client,
            banListId,
            "m.room.rule.user",
            "@old:localhost:9999",
        ...
        modifyingAgainEventId = await createPolicyRule(
            mjolnir.client,
            banListId,
            RULE_USER,
            "@old:localhost:9999",
    ...
    it("Will remove rules with old types when they are 'soft redacted' with a different but more recent event type.", async function () {
        ...
        let softRedactingEventId = await mjolnir.client.sendStateEvent(
            banListId,
            RULE_USER,
            `rule:${entity}`,
            {},
    ...
    it("A rule of the most recent type won't be deleted when an old rule is deleted for the same entity.", async function () {
        ...
        let updatedEventId = await createPolicyRule(
            mjolnir.client,
            banListId,
            RULE_USER,
            entity,
        ...
        assert.equal(
            banList.userRules.filter((rule) => rule.entity === entity).length,
            1,
            "The rule should still be active.",
        );
...
describe("Test: ACL updates will batch when rules are added in succession.", function () {
    it("Will batch ACL updates if we spam rules into a PolicyList", async function () {
        const mjolnir: Mjolnir = this.mjolnir!;
        const serverName: string = new UserID(await mjolnir.client.getUserId())
            .domain;
        const moderator = await newTestUser(this.config.homeserverUrl, {
            name: { contains: "moderator" },
        });
        ...
        await Promise.all(
            protectedRooms.map(async (room) => {
                ...
                const roomAcl = await mjolnir.client.getRoomStateEvent(
                    room,
                    "m.room.server_acl",
                    "",
                );
                assert.equal(
                    roomAcl?.deny?.length ?? 0,
                    0,
                    "There should be no entries in the deny ACL.",
                );
            }),
        );
        ...
        await mjolnir.policyListManager.watchList(
            MatrixRoomReference.fromRoomId(banListId),
        );
        const acl = new ServerAcl(serverName)
            .denyIpAddresses()
            .allowServer("*");
        ...
        assert.equal(
            list.serverRules.length,
            evilServerCount,
            `There should be ${evilServerCount} rules in here`,
        );
        ...
        await Promise.all(
            protectedRooms.map(async (room) => {
                ...
                if (!acl.matches(roomAcl)) {
                    assert.fail(
                        `Room ${room} doesn't have the correct ACL: ${JSON.stringify(
                            roomAcl,
                            null,
                            2,
                        )}`,
                ...
                assert.equal(
                    aclEventCount < 30 && aclEventCount > 2,
                    true,
                    "We should have sent less than 30 ACL events to each room because they should be batched",
                );
            }),
        );
    });
...
describe("Test: unbaning entities via the PolicyList.", function () {
    ...
    it("Will remove rules that have legacy types", async function () {
        ...
        const serverName: string = new UserID(await mjolnir.client.getUserId())
            .domain;
        const moderator: MatrixClient = await newTestUser(
        ...
        const protectedRoom = await moderator.createRoom({
            invite: [mjolnirId],
        });
        ...
        await moderator.sendStateEvent(
            banListId,
            "org.matrix.mjolnir.shortcode",
            "",
            { shortcode: "unban-test" },
        );
        await mjolnir.client.joinRoom(banListId);
        await mjolnir.policyListManager.watchList(
            MatrixRoomReference.fromRoomId(banListId),
        ...
        const protectedAcl = await mjolnir.client.getRoomStateEvent(
            protectedRoom,
            "m.room.server_acl",
            "",
        );
        if (!acl.matches(protectedAcl)) {
            assert.fail(
                `Room ${protectedRoom} doesn't have the correct ACL: ${JSON.stringify(
                    roomAcl,
                    null,
                    2,
                )}`,
        ...
        try {
            ...
            for (const server of [olderBadServer, newerBadServer]) {
                await getFirstReaction(
                    ...
                    "✅",
                    async () => {
                        return await moderator.sendMessage(
                            mjolnir.managementRoomId,
                            {
                                msgtype: "m.text",
                                body: `!mjolnir unban ${server} unban-test`,
                            },
                        );
                    },
                );
        ...
        await banList.updateList();
        assert.equal(banList.allRules.length, 0);
        const aclAfter = await mjolnir.client.getRoomStateEvent(
            protectedRoom,
            "m.room.server_acl",
            "",
        );
        assert.equal(
            aclAfter.deny.length,
            0,
            "Should be no servers denied anymore",
        );
    });
});
...
describe("Test: should apply bans to the most recently active rooms first", function () {
    it("Applies bans to the most recently active rooms first", async function () {
        ...
        await Promise.all(
            protectedRooms.map(async (room) => {
                const roomAcl = await mjolnir.client
                    ...
                    .catch((e) =>
                        e.statusCode === 404 ? { deny: [] } : Promise.reject(e),
                    );
                assert.equal(
                    roomAcl?.deny?.length ?? 0,
                    0,
        ...
        for (let i = protectedRooms.length - 1; i > 0; i--) {
            ...
            [protectedRooms[i], protectedRooms[j]] = [
                protectedRooms[j],
                protectedRooms[i],
            ];
        }
        ...
        for (const roomId of protectedRooms.slice().reverse()) {
            await moderator.sendMessage(roomId, {
                body: `activity`,
                msgtype: "m.text",
            });
        ...
        for (let i = 0; i < protectedRooms.length; i++) {
            assert.equal(
                mjolnir.protectedRoomsTracker.protectedRoomsByActivity()[i],
                protectedRooms[i],
            );
        }
        ...
        for (let i = 0; i < protectedRooms.length; i++) {
            assert.equal(
                aclRooms[i],
                protectedRooms[i],
                "The ACL should have been applied to the active rooms first.",
            );
        }
        ...
        for (const roomId of protectedRooms) {
            ...
            await getMessagesByUserIn(
                ...
                roomId,
                1,
                (events) => (roomAclEvent = events[0]),
            );
            const roomAcl = roomAclEvent!.content;
            if (!acl.matches(roomAcl)) {
                assert.fail(
                    `Room ${roomId} doesn't have the correct ACL: ${JSON.stringify(
                        roomAcl,
                        null,
                        2,
                    )}`,
...
describe("Test: AccessControlUnit interaction with policy lists.", function () {
    it("The AccessControlUnit correctly reflects the policies that have been set in its watched lists.", async function () {
        ...
        const policyList = new PolicyList(
            policyListId,
            Permalinks.forRoom(policyListId),
            mjolnir.client,
        );
        const aclUnit = new AccessControlUnit([policyList]);
        assertAccess(
            Access.Allowed,
            aclUnit.getAccessForUser(
                "@anyone:anywhere.example.com",
                "CHECK_SERVER",
            ),
        ...
        assertAccess(
            Access.Allowed,
            aclUnit.getAccessForUser("@someone:matrix.org", "CHECK_SERVER"),
        );
        assertAccess(
            Access.NotAllowed,
            aclUnit.getAccessForServer("anywhere.else.example.com"),
        );
        assertAccess(
            Access.NotAllowed,
            aclUnit.getAccessForUser(
                "@anyone:anywhere.else.example.com",
                "CHECK_SERVER",
            ),
        );
        assertAccess(
            Access.Banned,
            aclUnit.getAccessForServer("bad.example.com"),
        );
        assertAccess(
            Access.Banned,
            aclUnit.getAccessForUser("@anyone:bad.example.com", "CHECK_SERVER"),
        );
        // They're not allowed in the first place, never mind that they are also banned.
        assertAccess(
            Access.NotAllowed,
            aclUnit.getAccessForServer("meow.ddns.example.com"),
        );
        assertAccess(
            Access.NotAllowed,
            aclUnit.getAccessForUser(
                "@anyone:meow.ddns.example.com",
                "CHECK_SERVER",
            ),
        );
        ...
        assertAccess(
            Access.Allowed,
            aclUnit.getAccessForUser("@spam:matrix.org", "CHECK_SERVER"),
        );
        await createPolicyRule(
            mjolnir.client,
            policyListId,
        ...
        await mjolnir.policyListManager.watchList(
            MatrixRoomReference.fromPermalink(policyList.roomRef),
        );
        ...
        assert.equal(
            roomAcl?.deny?.length ?? 0,
            2,
            "There should be two entries in the deny ACL.",
        );
        ...
        assert.equal(
            changes.length,
            5,
            "The rules should have correctly been removed",
        );
        assertAccess(Access.Allowed, aclUnit.getAccessForServer("matrix.org"));
        assertAccess(
            Access.Allowed,
        ...
        const roomAclAfter = await mjolnir.client.getRoomStateEvent(
            protectedRoom,
            "m.room.server_acl",
            "",
        );
        assert.equal(
            roomAclAfter.deny?.length ?? 0,
            0,
            "There should be no entries in the deny ACL.",
        );
        assert.equal(
            roomAclAfter.allow?.length ?? 0,
            1,
            "There should be 1 entry in the allow ACL.",
        );
    ...
    it("removing a rule from a different list will not clobber anything.", async function () {
        ...
        assertAccess(
            Access.Banned,
            aclUnit.getAccessForServer(banMeServer),
            "Should still be banned at this point.",
        );
        await removePolicyRule(
            mjolnir.client,
            policyLists[1].roomId,
            RULE_SERVER,
            banMeServer,
        );
        await Promise.all(policyLists.map((list) => list.updateList()));
        assertAccess(
            Access.Allowed,
            aclUnit.getAccessForServer(banMeServer),
            "Should not longer be any rules",
        );
    });
});
...
describe("Test: Creating policy lists.", function () {
    it("Will automatically invite and op users from invites", async function () {
        ...
        const testUsers = await Promise.all(
            [...Array(2)].map((_) =>
                newTestUser(this.config.homeserverUrl, {
                    name: { contains: "moderator" },
                }),
            ),
        );
        const invite = await Promise.all(
            testUsers.map((client) => client.getUserId()),
        );
        ...
        const powerLevelEvent = await mjolnir.client.getRoomStateEvent(
            policyListId,
            "m.room.power_levels",
            "",
        );
        assert.equal(
            Object.keys(powerLevelEvent.users ?? {}).length,
            invite.length + 1,
        );
        // Check create event for MSC3784 support.
        const createEvent = await mjolnir.client.getRoomStateEvent(
            policyListId,
            "m.room.create",
            "",
        );
...
describe.only("Test: Continue to ban other marked members when one member cannot be banned", function () {
    it("Failing to ban a moderator should not stop other members being banned.", async function (this: MjolnirTestContext) {
        ...
        const spamUser = await newTestUser(this.config.homeserverUrl, {
            name: { contains: "mx-spammer" },
        });
        const rudeUser = await newTestUser(this.config.homeserverUrl, {
            name: { contains: "mx-rude" },
        });
        const sillyModerator = await newTestUser(this.config.homeserverUrl, {
            name: { contains: "mx-moderator" },
        });
        await Promise.all(
            [spamUser, rudeUser, sillyModerator].map((c) =>
                c.joinRoom(protectedRoom),
            ),
        );
        await moderator.setUserPowerLevel(
            await sillyModerator.getUserId(),
            protectedRoom,
            100,
        );
        ...
        const assertMembership = (
            ...
            assert.equal(
                members.find((e) => e.stateKey === userId)?.membership,
                membership,
            );
        };
