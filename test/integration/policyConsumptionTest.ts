...
async function currentRules(
    mjolnir: Mjolnir,
): Promise<{ start: object; stop: object; since: string }> {
...
async function waitForRuleChange(mjolnir: Mjolnir, thunk: any): Promise<void> {
    ...
    LogService.debug(
        "policyConsumptionTest",
        `Rules before we wait for them to change: ${JSON.stringify(
            rules,
            null,
            2,
        )}`,
    ...
    LogService.debug(
        "policyConsumptionTest",
        `Rules after they have changed: ${JSON.stringify(rules, null, 2)}`,
    );
}
...
describe("Test: that policy lists are consumed by the associated synapse module", function () {
    this.afterEach(async function () {
        if (this.config.web.ruleServer.enabled) {
            ...
            LogService.debug(
                "policyConsumptionTest",
                `Rules at end of test ${JSON.stringify(
                    await currentRules(this.mjolnir),
                    null,
                    2,
                )}`,
            );
    ...
    this.beforeAll(async function () {
        ...
        if (!config?.web?.ruleServer?.enabled) {
            LogService.warn(
                "policyConsumptionTest",
                "Skipping policy consumption test because the ruleServer is not enabled",
            );
            this.skip();
        }
    });
    it("blocks users in antispam when they are banned from sending messages and invites serverwide.", async function () {
            ...
            await getFirstReaction(
                ...
                async () => {
                    return await moderator.sendMessage(
                        ...
                        {
                            msgtype: "m.text",
                            body: `!mjolnir ban ${badUserId} ${banList}`,
                        },
                    );
                },
            );
        ...
        await assert.rejects(
            badUser.sendMessage(unprotectedRoom, {
                msgtype: "m.text",
                body: "test",
            }),
        ...
        assert.ok(
            await moderator.sendMessage(unprotectedRoom, {
                msgtype: "m.text",
                body: "test",
            }),
            ...
            await getFirstReaction(
                ...
                async () => {
                    return await moderator.sendMessage(
                        ...
                        {
                            msgtype: "m.text",
                            body: `!mjolnir unban ${badUserId} ${banList}`,
                        },
                    );
                },
            );
    ...
    it("Test: Cannot send message to a room that is listed in a policy list and cannot invite a user to the room either", async function () {
        ...
        await badUser.sendMessage(badRoom, {
            msgtype: "m.text",
            body: "Very Bad Stuff in this room",
        });
        await waitForRuleChange(this.mjolnir, async () => {
            await getFirstReaction(
                this.mjolnir.client,
                ...
                async () => {
                    return await moderator.sendMessage(
                        ...
                        {
                            msgtype: "m.text",
                            body: `!mjolnir ban ${badRoom} ${banList}`,
                        },
                    );
                },
            );
        ...
        assert.ok(
            await badUser.sendMessage(unrelatedRoom, {
                msgtype: "m.text.",
                body: "hey",
            }),
            ...
            await getFirstReaction(
                ...
                async () => {
                    return await moderator.sendMessage(
                        ...
                        {
                            msgtype: "m.text",
                            body: `!mjolnir unban ${badRoom} ${banList}`,
                        },
                    );
                },
            );
            ...
            await getFirstReaction(
                ...
                async () => {
                    return await moderator.sendMessage(
                        ...
                        {
                            msgtype: "m.text",
                            body: `!mjolnir ban ${targetRoom} ${banList}`,
                        },
                    );
                },
            );
            ...
            await getFirstReaction(
                ...
                async () => {
                    return await moderator.sendMessage(
                        ...
                        {
                            msgtype: "m.text",
                            body: `!mjolnir unwatch #${banList}:localhost:9999`,
                        },
                    );
                },
            );
