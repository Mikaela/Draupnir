...
import {
    DetectFederationLag,
    LAG_STATE_EVENT,
} from "../../src/protections/DetectFederationLag";
...
describe("Test: DetectFederationLag protection", function () {
    ...
    beforeEach(async function () {
        ...
        await this.mjolnir.protectionManager.enableProtection(
            "DetectFederationLag",
        );
        ...
        const SETTINGS = {
            ...
            numberOfLaggingFederatedHomeserversEnterWarningZone:
                NUMBER_OF_LAGGING_FEDERATED_HOMESERVERS_ENTER_WARNING_ZONE,

            localHomeserverLagEnterWarningZone:
                LOCAL_HOMESERVER_LAG_ENTER_WARNING_ZONE_MS,
            localHomeserverLagExitWarningZone:
                LOCAL_HOMESERVER_LAG_EXIT_WARNING_ZONE_MS,

            federatedHomeserverLagEnterWarningZone:
                FEDERATED_HOMESERVER_LAG_ENTER_WARNING_ZONE_MS,
            federatedHomeserverLagExitWarningZone:
                FEDERATED_HOMESERVER_LAG_EXIT_WARNING_ZONE_MS,
        };
        for (let key of Object.keys(SETTINGS)) {
            this.detector.settings[key].setValue(SETTINGS[key]);
        }
        this.localDomain = new UserID(
        ...
        this.simulateLag = async (
            ...
            for (let i = 0; i < SAMPLE_SIZE; ++i) {
                ...
                await this.detector.handleEvent(
                    this.mjolnir,
                    this.protectedRoomId,
                    {
        ...
        this.getAlertEvent = async () => {
            try {
                let event = await this.mjolnir.client.getRoomStateEvent(
        ...
        this.getCommandStatus = async () => {
            const protectedRoomReply = await getFirstReply(
                this.mjolnir.client,
                this.mjolnir.managementRoomId,
                () => {
            ...
            const globalReply = await getFirstReply(
                this.mjolnir.client,
                this.mjolnir.managementRoomId,
                () => {
            ...
            const protectedRoomStatsStr =
                protectedRoomReply.content.body.match(RE_STATS)[0];
            const globalStatsStr = globalReply.content.body.match(RE_STATS)[0];
            return {
                protectedRoomStats: protectedRoomStatsStr
                    ? JSON.parse(protectedRoomStatsStr)
                    : null,
    ...
    let simulateDate = (start: Date, progress: number = SAMPLE_SIZE) =>
        new Date(
            start.getTime() + (2 * progress * BUCKET_DURATION_MS) / SAMPLE_SIZE,
        );
    ...
    it("DetectFederationLag doesn't detect lag when there isn't any", async function () {
        ...
        for (let multiplier of MULTIPLIERS) {
            ...
            assert.equal(
                await this.getAlertEvent(),
                null,
                `We have sent lots of local pseudo-events with a small lag of ${LAG}, there should be NO alert`,
            );
        }
        ...
        for (let multiplier of MULTIPLIERS) {
            const LAG =
                multiplier * FEDERATED_HOMESERVER_LAG_ENTER_WARNING_ZONE_MS;
            await this.simulateLag(REMOTE_SENDERS, LAG, start);
            assert.equal(
                await this.getAlertEvent(),
                null,
                `We have sent lots of remote pseudo-events with a small lag of ${LAG}, there should be NO alert`,
            );
        }

        const { protectedRoomStats, globalStats } =
            await this.getCommandStatus();
        assert.ok(protectedRoomStats, "We should see stats for our room");
        assert.ok(
            protectedRoomStats.min >= 0,
            `min ${protectedRoomStats.min} >= 0`,
        );
        ...
        assert.deepEqual(globalStats, {
            [this.protectedRoomId]: protectedRoomStats,
        });
    });
    ...
    it("DetectFederationLag detects lag on local homeserver", async function () {
        ...
        await this.simulateLag(
            SENDERS,
            1.5 * LOCAL_HOMESERVER_LAG_ENTER_WARNING_ZONE_MS,
            start,
        );
        ...
        assert.equal(
            JSON.stringify(lagEvent.domains),
            JSON.stringify([this.localDomain]),
            "Lag report should mention only the local domain",
        );
        assert.equal(
            lagEvent.roomId,
            this.protectedRoomId,
            "Lag report should mention the right room",
        );
        assert(
            new Date(lagEvent.since) >= start,
            "Lag report should have happened since `now`",
        );
        assert(
            new Date(lagEvent.since) < stop,
            "Lag should have been detected before the end of the bombardment",
        );
        ...
        await this.simulateLag(
            SENDERS_2,
            0.75 * LOCAL_HOMESERVER_LAG_EXIT_WARNING_ZONE_MS,
            start2,
        );
        ...
        assert.equal(
            await this.getAlertEvent(),
            null,
            "The alert should now be rescinded",
        );
    ...
    it("DetectFederationLag doesn't report lag when only one federated homeserver lags", async function () {
        ...
        await this.simulateLag(
            SENDERS,
            1.5 * FEDERATED_HOMESERVER_LAG_ENTER_WARNING_ZONE_MS,
            start,
        );
        ...
        assert.equal(
            lagEvent,
            null,
            "With only one federated homeserver lagging, we shouldn't report any lag",
        );
    });
    ...
    it("DetectFederationLag reports lag when three federated homeservers lag", async function () {
        ...
        assert(lagEvent, "Local lag should be reported");
        assert.equal(
            JSON.stringify(lagEvent.domains.sort()),
            JSON.stringify([
                "left.example.com",
                "middle.example.com",
                "right.example.com",
        ...
        await this.simulateLag(
            SENDERS_2,
            0.75 * FEDERATED_HOMESERVER_LAG_EXIT_WARNING_ZONE_MS,
            start2,
        );
