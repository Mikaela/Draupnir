...
describe("Ban propagation test", function () {
    it("Should be enabled by default", async function () {
        ...
        expect(
            mjolnir.protectionManager.getProtection("BanPropagationProtection")
                ?.enabled,
        ).toBeTruthy();
    });
    it("Should prompt to add bans to a policy list, then add the ban", async function () {
        ...
        await mjolnir.policyListManager.watchList(
            MatrixRoomReference.fromRoomId(policyListId),
        );
        ...
        const promptEvent = await getFirstEventMatching({
            ...
            predicate: function (event: any): boolean {
                return (event["content"]?.["body"] ?? "").startsWith(
                    "The user",
                );
            },
        });
        ...
        const rules = policyList.rulesMatchingEntity(
            "@test:example.com",
            RULE_USER,
        );
