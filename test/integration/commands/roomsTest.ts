        ...
        try {
            ...
            let protectedRoomsMessage = await getFirstReply(
                ...
                async () => {
                    return await moderator.sendMessage(
                        this.mjolnir.managementRoomId,
                        { msgtype: "m.text", body: `!mjolnir rooms` },
                    );
                },
            );
            assert.equal(
                ...
                true,
                "There should be one protected room",
            );
            await getFirstReaction(
                moderator,
                this.mjolnir.managementRoomId,
                ...
                async () => {
                    return await moderator.sendMessage(
                        ...
                        {
                            msgtype: "m.text",
                            body: `!mjolnir rooms remove ${targetRoom}`,
                        },
                    );
                },
            );
            ...
            assert.equal(
                protectedRoomsMessage["content"]?.["body"]?.includes("0"),
                true,
                "There room should no longer be protected",
            );
        } finally {
            moderator.stop();
        }
