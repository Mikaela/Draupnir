...
describe("Test: The make admin command", function () {
    it("Mjölnir make the bot self room administrator", async function (this: MjolnirTestContext) {
        ...
        await moderator.sendMessage(mjolnir.managementRoomId, {
            msgtype: "m.text.",
            body: `!mjolnir rooms add ${targetRoom}`,
        });
        await userA.joinRoom(targetRoom);
        const powerLevelsBefore = await mjolnir.client.getRoomStateEvent(
            targetRoom,
            "m.room.power_levels",
            "",
        );
        ...
        assert.notEqual(
            powerLevelsBefore["users"][mjolnirUserId],
            100,
        ...
        await getFirstReaction(
            ...
            async () => {
                return await moderator.sendMessage(mjolnir.managementRoomId, {
                    msgtype: "m.text",
                    body: `!mjolnir hijack room ${targetRoom} ${mjolnirUserId}`,
                });
            },
        );

        const powerLevelsAfter = await mjolnir.client.getRoomStateEvent(
            targetRoom,
            "m.room.power_levels",
            "",
        );
        assert.equal(
            powerLevelsAfter["users"][mjolnirUserId],
            100,
