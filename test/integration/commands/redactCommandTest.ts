...
describe("Test: The redaction command", function () {
            ...
            function (events) {
                events.map((e) => {
                    if (e.type === "m.room.member") {
                        ...
                        throw new Error(
                            `This event should have been redacted: ${JSON.stringify(
                                e,
                                null,
                                2,
                            )}`,
    ...
    it("Redacts a single event when instructed to.", async function () {
        ...
        let eventToRedact = await badUser.sendMessage(targetRoom, {
            msgtype: "m.text",
            body: "Very Bad Stuff",
        });
            ...
            await getFirstReaction(
                ...
                async () => {
                    return await moderator.sendMessage(
                        ...
                        {
                            msgtype: "m.text",
                            body: `!mjolnir redact https://matrix.to/#/${encodeURIComponent(
                                targetRoom,
                            )}/${encodeURIComponent(eventToRedact)}`,
                        },
                    );
                },
            );
